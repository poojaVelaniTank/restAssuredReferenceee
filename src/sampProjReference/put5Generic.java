...
public class put5Generic {
	public static void main(String[] args) {
		...
		int statusCode = given().header("content-Type", "application/json").body(requestBody).when().put("/api/users/2").then().
				extract().statusCode();
		System.out.println(statusCode);
		String responsebody = given().header("content-Type", "application/json").body(requestBody).when().put("/api/users/2").then()
